...
@enderror" />
        ...
        <div id="newsImages" data-tab-content class="hidden space-y-6">
            ...
            <form action="{{ route('admin.news.images.store', $news->id) }}" method="POST"
                enctype="multipart/form-data" class="mb-6 p-4 border rounded-md dark:border-stone-700">
                @csrf
                <div>
                    <label for="new_images" class="block text-sm font-medium text-slate-700 dark:text-slate-300">Tambah
                        Gambar Baru (Bisa Pilih Banyak)</label>
                ...
                <div class="mt-4 flex justify-end">
                    <button type="submit"
                        class="px-4 py-2 text-sm font-medium text-white bg-blue-600 rounded-md hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 dark:focus:ring-offset-stone-800">
                        Upload Gambar
                    </button>
                </div>
            </form>
                    ...
                    <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
                        @foreach ($news->images as $image)
                            <div class="relative group border rounded-md dark:border-stone-700 p-2">
                                <img src="{{ Storage::url($image->path) }}" alt="News image"
                                    class="rounded-md object-cover w-full h-32">
                                <form action="{{ route('admin.news.images.destroy', $image->id) }}" method="POST"
                                    class="absolute top-1 right-1">
                                    @csrf
                                    @method('DELETE')
                                    <button type="button" onclick="confirmDelete(this)"
                                        class="p-1.5 bg-red-600 text-white rounded-full opacity-75 group-hover:opacity-100 hover:bg-red-700 transition-opacity text-xs">
        ...
        <script>
            ...
            document.addEventListener('DOMContentLoaded', function() {
                ...
                if (tabButtons.length > 0) {
                    ...
                    if (storedTargetId) {
                        const foundButton = Array.from(tabButtons).find(btn => btn.dataset.tabTarget ===
                            storedTargetId);
                        if (foundButton) {
