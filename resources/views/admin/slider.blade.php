<x-admin-layout :pageTitle="'Kelola Slider'">
    ...
    <div class="mb-6">
        ...
        <p class="text-slate-600 dark:text-white">Unggah dan ganti gambar untuk slider di halaman utama. Terdapat 5 slot gambar yang dapat Anda kelola.</p>
    </div>
...
@foreach ($currentSlidersData as $sliderData)
    ...
    <div class="bg-red-700 dark:bg-stone-500 p-6 rounded-xl shadow-lg hover:shadow-2xl transition-shadow duration-300 ease-in-out">
        <h3 class="text-lg font-semibold text-white dark:text-slate-100 mb-4">Slider Gambar #{{ $slotNumber }}</h3>

        {{-- Pratinjau Gambar --}}
        <div class="mb-4 aspect-[16/9] bg-red-600 dark:bg-stone-600 rounded-md flex items-center justify-center overflow-hidden">
            <img id="sliderPreview{{ $slotNumber }}" src="{{ $sliderData['image_url'] }}" alt="Pratinjau Slider {{ $slotNumber }}" class="w-full h-full object-cover">
        </div>

        {{-- Form untuk mengganti gambar --}}
        {{-- PASTIKAN ACTION INI BENAR --}}
        <form action="{{ route('admin.sliders.update', ['slot_number' => $slotNumber]) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PATCH') {{-- Ini akan mengirimkan request sebagai PATCH --}}

            <div class="mb-4">
                <label for="sliderImageInput{{ $slotNumber }}" class="sr-only">Pilih Gambar untuk Slider {{ $slotNumber }}</label>
                ...
                @if ($errors->hasBag($errorKey) && $errors->getBag($errorKey)->has('slider_image'))
                    <p class="mt-1 text-xs text-yellow-300">{{ $errors->getBag($errorKey)->first('slider_image') }}</p>
                @endif
            </div>
