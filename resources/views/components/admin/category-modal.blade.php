@props([
    ...
    'formId',
    'submitButtonText',
    'isEdit' => false, // Consistent with gallery-modal, can be true by default if always edit
])
...
<div id="{{ $modalId }}"
    ...
    <div class="flex items-center justify-center min-h-screen p-4">
        <div class="bg-white dark:bg-stone-800 rounded-lg shadow-xl p-6 w-full max-w-md transform transition-all duration-300 ease-in-out scale-95 opacity-0"
            id="{{ $modalId }}Content">
            ...
            <form id="{{ $formId }}" class="mt-4 space-y-4" method="post">
            @csrf
            @if ($isEdit)
                @method('PUT')
                <input type="hidden" name="id" id="{{ $isEdit ? 'edit' : 'create' }}_category_id_hidden">
            @endif

            <div>
                <label for="{{ $isEdit ? 'edit' : 'create' }}_category_name"
                    class="block text-sm font-medium text-slate-700 dark:text-slate-300">Nama Kategori</label>
                <input type="text" name="name" id="{{ $isEdit ? 'edit' : 'create' }}_category_name"
                    ...
                    required>
                <span class="text-xs text-red-500 hidden" id="{{ $isEdit ? 'edit' : 'create' }}_category_name_error"></span>
            </div>
